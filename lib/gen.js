...
const transferFilesToData = async (dirUrl, tags = [], formatBlogs = []) => {
    ...
    for(let file of files) {
        ...
        if (stat && stat.isDirectory()) {
            await transferFilesToData(filePath, tags.concat(file), formatBlogs);
        } else {
...
const gen = async (config) => {
    const res = await transferFilesToData(`${process.cwd()}${config.mdPath}`);
